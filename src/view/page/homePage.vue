<template>
	<div v-loading="loading" element-loading-text="程序执行中" element-loading-spinner="el-icon-loading"
		...
		<!-- 企业详情页组件 TODO: 拆分 -->
		<div>
			<Modal v-model="modalCompanyDetail" @on-cancel="beforeModalCancel" :scrollable="scrollableFlag" ref="detailModal"
				:styles="{top: '10%',overflow:'auto'}" width="70%" :draggable="true" height="20%" class-name="center-modal"
				...
				<div>
					...
					<div class="lct" v-show="imgBoxShow" style="overflow: auto">
						<ProcessGraph :companyCodeForGraph="companyCodeForGraph" :companyName="exlname" v-if="modalCompanyDetail && imgBoxShow" />
					</div>
					<el-tabs v-model="msgValue" type="card" :value="lineTabIndex" @tab-click="clickLineTabs()">
						<el-tab-pane v-for="(lineTab,index) in lineTabs" :key="index" :label="`${lineTab.deviceName}`" :name="`${lineTab.deviceName}`">
							...
							<Tabs type="card" @on-click="clickLineDetailInfoTabs" :animated="false" :value="dateRangIndex"
								...
								<TabPane label="实时数据" style="overflow:visible;">
									<Button type="primary" style="margin-bottom:10px;" @click="changeEcharts(1)">折线图</Button>
									<Button type="primary" style="margin-bottom:10px;margin-left:5px;" @click="changeEcharts(2)">时序图</Button>
									<!-- 折线图 -->
									...
									<Tabs type="card" :animated="false" width="100%" style="overflow:visible;" v-show="echartsNumber==2">
										<TabPane :label=pmname :disabled="histogramHourEnegyDisabled" style="overflow:visible;">
											<div style="width:100%">
												<timing :init-value="lineHourEnegyData"></timing>
												<Spin size="large" fix v-if="histogramLoading"></Spin>
											</div>
										</TabPane>
									</Tabs>
										...
										<Row>
											...
											<FormItem label="选择日期 : ">
												<Row style="width:500px">
													<Col span="24">
													<FormItem prop="date">
														<DatePicker type="datetime" placeholder="请选择开始日期 " style="width: 40%;display:inline-block;"
															@on-change="getstartData"></DatePicker>~
														<DatePicker type="datetime" placeholder="请选择截止日期 " style="width: 40%;display:inline-block;"
								...
								<TabPane label="月查询">
									<Form :label-width="80" style="height: 35px;">
										<FormItem label="选择年月 : ">
											<Row>
												<Col span="5">
												<Cascader :data="yearMonthList" @on-change="onYearMonthChange" v-model="yearMonthValue">
												</Cascader>
												</Col>
											</Row>
										</FormItem>
