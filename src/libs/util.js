...
export const getMd5 = (str) => {
  ...
  md5.update(str)
  let rtnStr = md5.digest('hex')
  return rtnStr
}

export const setUserMap = (userMap) => {
  // Cookies.set(USER_MAP, userMap, {'max-age': config.maxAge || 10800})
  Cookies.set(USER_MAP, userMap)
}

export const getUserMap = () => {
  const userMap = Cookies.get(USER_MAP)
  if (userMap) return JSON.parse(userMap)
  else return false
}

export const setToken = (token) => {
  // Cookies.set(TOKEN_KEY, token, {expires: config.cookieExpires || 1})
  // Cookies.set(TOKEN_KEY, token, {'max-age': config.maxAge || 10800})
